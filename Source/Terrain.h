...
class Terrain : public BaseModel
{
	...
	UINT numInd = 0;
public:
	Terrain(ID3D11Device *device, ID3D11DeviceContext*context, int width, int height, ID3D11Texture2D*tex_height, 
		ID3D11Texture2D*tex_normal,  Effect *_effect, Material *_materials[] = nullptr, int _numMaterials = 0, 
		ID3D11ShaderResourceView **textures = nullptr, int numTextures = 0) : BaseModel(device, _effect, _materials, _numMaterials, textures, numTextures){ init(device, context,width,height, tex_height, tex_normal); };
